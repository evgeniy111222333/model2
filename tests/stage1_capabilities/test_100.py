"""
BCS Stage 1 Capability Test 100: Optim: Semantic Query Readout Init
Description: Ensure semantic readout layers initialize correctly
...
def run_test():
    print("Running capability test 100: Optim: Semantic Query Readout Init")
    
    # 1. Generate Input Data
    data = b'ABCDEFGH' * 50
    
    # 2. Initialize Model
    # Config parameters: {'use_semantic_readout': True, 'n_active_bytes': 32}
    model = create_model(**{'use_semantic_readout': True, 'n_active_bytes': 32})
    ...
    metrics = {
        "test_id": 100,
        "test_name": "Optim: Semantic Query Readout Init",
        "description": "Ensure semantic readout layers initialize correctly",
        "success": False
    }
    ...
    out_path = f"test_results/cap_test_100.json"
