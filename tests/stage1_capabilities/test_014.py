"""
BCS Stage 1 Capability Test 014: Field: dt Time-Step Scaling Sensitivity
Description: Check how time-step changes (dt=0.3 vs default) affect stability
...
def run_test():
    print("Running capability test 014: Field: dt Time-Step Scaling Sensitivity")
    
    # 1. Generate Input Data
    data = b'ABCDEFGH' * 50
    
    # 2. Initialize Model
    # Config parameters: {'dt': 0.3, 'n_active_bytes': 32}
    model = create_model(**{'dt': 0.3, 'n_active_bytes': 32})
    ...
    metrics = {
        "test_id": 14,
        "test_name": "Field: dt Time-Step Scaling Sensitivity",
        "description": "Check how time-step changes (dt=0.3 vs default) affect stability",
        "success": False
    }
    ...
    out_path = f"test_results/cap_test_014.json"
