"""
BCS Stage 1 Capability Test 018: Field: Feed Rate F_base Dynamics
Description: Evaluate field activation levels (u_mean) under higher feed rate F_base
...
def run_test():
    print("Running capability test 018: Field: Feed Rate F_base Dynamics")
    
    # 1. Generate Input Data
    data = b'ABCDEFGH' * 50
    
    # 2. Initialize Model
    # Config parameters: {'F_base': 0.06, 'n_active_bytes': 32}
    model = create_model(**{'F_base': 0.06, 'n_active_bytes': 32})
    ...
    metrics = {
        "test_id": 18,
        "test_name": "Field: Feed Rate F_base Dynamics",
        "description": "Evaluate field activation levels (u_mean) under higher feed rate F_base",
        "success": False
    }
    ...
    out_path = f"test_results/cap_test_018.json"
